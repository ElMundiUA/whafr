...
def _crawl_factory(args: dict[str, Any]) -> Connector:
    from lighthouse.connectors.crawl import CrawlConnector

    return CrawlConnector(
        root=args["root"],
        ...
        wait_for_dynamic=int(args.get("wait_for_dynamic", 0)),
    )
